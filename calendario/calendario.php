<?php
function calcula_numero_dia_semana($ano,$mes,$dia){
	$numerodiasemana = date('w', mktime(0,0,0,$ano,$mes,$ano));
...
function mostrar_calendario($mes,$ano){
	...
	for ($i=0;$i<7;$i++){
		if ($i < $numero_dia){
			...
			echo "<td class='diavalido'><span><a href='javascript:devuelveFecha($ano,$mes,$dia_actual)'". dame_estilo($dia_actual) .">$dia_actual</a><span></td>";
			$dia_actual++;
		}
	}
	echo "</tr>";
